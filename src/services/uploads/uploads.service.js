...
const execUpload = multer({
  ...
  fileFilter: function (req, file, callback) { // 限制上傳檔案格式 只能圖片
    ...
    // 可以上傳docx
    if (file.mimetype === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') { return callback(null, true) }

    // 可以上傳excel
    if (file.mimetype.includes('excel') || file.mimetype.includes('spreadsheetml')) { return callback(null, true) }

    // 可以上傳pdf
    if (file.mimetype.includes('pdf') || file.mimetype.includes('application/pdf')) { return callback(null, true) }
